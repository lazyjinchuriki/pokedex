...
const filterPokemonsByType = (selectedType) => {
	const cards = document.querySelectorAll('.card');

	cards.forEach((card) => {
		const cardTypes = Array.from(card.querySelectorAll('.poke__type__bg')).map(
			(type) => type.classList[1]
		);
		if (selectedType === 'all' || cardTypes.includes(selectedType)) {
